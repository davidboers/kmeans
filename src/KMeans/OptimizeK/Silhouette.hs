module KMeans.OptimizeK.Silhouette
    ...
    , meanSilhouetteScore
    , silhouetteScore
    , meanIntraClusterDistance
    , meanNearestClusterDistance
    ) where
...
meanSilhouetteScore seed points k =
    let clusters = kMeansStatic seed k points
     in avgDouble $ map (silhouetteScore clusters) points
...
silhouetteScore clusters p =
    let cluster = getCluster clusters p
        ...
        b = meanNearestClusterDistance clusters p
     in (b - a) / max a b
